...
「現在のバリュー局面分析」を**約500〜650字**の日本語で書いてください。

【必ず含める観察軸】
1. 6カテゴリ全体を俯瞰した観察(現在の市場でバリュー的に魅力的なセクター・特徴)
2. 注目すべき1〜3銘柄を固有名で言及
3. **マクロ・原材料リスクの言及**(以下を該当箇所で具体的に触れること):
   - ナフサ・原油高騰 → 化学・素材・建材樹脂・塗料・接着剤・断熱材セクターへの粗利圧迫
   - 鋼材・セメント価格 → ゼネコン・建設の固定価格契約での原価転嫁遅延
   - 木材・住宅金利 → 住宅・不動産の需要側・コスト側の二面リスク
   - エネルギー価格 → 製造業・素材全般の電力・燃料コスト圧力
4. **セクター集中度の評価**(総合スコア上位や AI ピック想定構成が特定セクターに偏っていないか — 4銘柄以上が同一セクターなら明示的に注意喚起)
5. 「バリュートラップの可能性」として注意喚起(高配当・低PBRに潜むリスク)
6. 投資勧誘ではなく、観察・分析のトーン
7. **見出し記号(#や##や**)は一切使わず、自然な段落で**
...
【セクター分散ルール(厳守)】
- **同一セクター(建設・資材、素材・化学、不動産、機械、食品、医薬品、情報通信、金融、商社など)から 3銘柄まで**。4銘柄目以上は別セクターから取ること。
- 7銘柄選ぶ場合、**最低3セクター以上に分散**させること。
- セクター集中はマクロショック(ナフサ高騰、住宅金利上昇、円高、米国景気減速など)で一斉に被弾するため、分散はバリュー投資のリスク管理上必須。

【リスク欄の必須記載事項】
各銘柄の `risk` 欄には、**「景気感応度」や「市場環境悪化」といった一般論は禁止**。代わりに以下のうち**該当する具体的マクロリスクを必ず1つ以上明示**すること:
- 建設・ゼネコン: 鋼材・セメント・労務費上昇 → 固定価格契約での原価転嫁遅延
- 不動産・住宅: 住宅金利・木材・建材樹脂(ナフサ系)コスト
- 化学・素材: ナフサ・原油・電力エネルギーコスト
- 自動車部品: 鋼材・アルミ・半導体不足
- 鉄鋼・耐火材: 鉄鉱石・原料炭、顧客の高炉稼働率
- 食品: 穀物・包材(ナフサ系プラスチック)・燃料費
- その他は当該業界固有の主原料・主コストを具体的に
...
- `ticker`: 文字列(例 "1911.T")
- `name`: 銘柄名
- `sector`: セクター名(分散チェック用)
- `appeal`: 80〜120字の選定理由(具体的数値で)
- `growth_evidence`: 売上・利益の継続性根拠(40〜80字)
- `risk`: **具体的マクロリスク + バリュートラップ可能性**(60〜100字、上記必須記載事項に従う)
