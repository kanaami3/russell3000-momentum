...
def build_overall_prompt(data: dict) -> str:
    rk = data["rankings"]
    def lst(name, n=10):
...
【成果物1: 総合解説(narrative)】
約400〜500字の日本語で「現在のバリュー局面分析」を書いてください。
...
【成果物2: AIが選ぶ高品質バリュー銘柄 5〜7個(JSON形式)】

総合スコア + 個別カテゴリから、**配当 + 割安 + 売上継続 + 成長見込み** の4条件にバランス良く該当する銘柄を 5〜7個ピックしてください。
...
- `target_observation`: 「現在PBR X倍は過去Y年で割安水準」「配当利回り X% は同業平均より高い」等の客観観察(40〜80字)

JSONブロックは ```json で囲んでください。narrativeの後に配置。
...
def main() -> int:
    ...
    # Overall + AI picks
    print(f"Claude → overall + AI picks ({MODEL})...", file=sys.stderr)
    try:
        resp = client.messages.create(
            model=MODEL,
            max_tokens=MAX_TOKENS_OVERALL,
            messages=[{"role": "user", "content": build_overall_prompt(data)}],
        )
        text = "".join(b.text for b in resp.content if b.type == "text").strip()
        narrative, picks = parse_picks(text)
        data["overall_commentary"] = narrative
        data["ai_picks"] = picks
        total_in += resp.usage.input_tokens
        total_out += resp.usage.output_tokens
        print(f"  narrative: {len(narrative)} chars, AI picks: {len(picks)}", file=sys.stderr)
