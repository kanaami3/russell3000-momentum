...
- `entry`: 寄付エントリー戦略の目安 (例: "寄付直後の押し目 1,920〜1,950 円拾い")  ← 表示用テキスト
- `target`: 利食い目安 — 翌日持越しの場合は「本日 target / 翌日 target」両方を示す  ← 表示用テキスト
- `risk`: 撤退ライン or 想定リスク (損切り価格、想定リスク要因)  ← 表示用テキスト

**さらに、上記の文章とは別に、機械処理用の「純粋な数値フィールド」を必ず出力すること(円単位の数字のみ、カンマ・単位・記号なし):**
- `entry_low`: number  ← エントリー想定レンジ下限(円)
- `entry_high`: number ← エントリー想定レンジ上限(円)。レンジでなければ entry_low と同値
- `stop`: number       ← 損切り価格(円)。**必ず entry_low より低い値**にすること
- `target_today`: number ← 本日の利食い目標(円)
- `target_next`: number or null ← 翌日の利食い目標(円)。日中決済なら null

【数値フィールドの厳守事項】
- RSI・出来高・時刻(例「9時30分」)・パーセント(例「-2〜3%」)など、**株価以外の数字を絶対に入れない**こと。
- 全て当該銘柄の現実的な株価水準(引け値の近辺)に整合させること。
- entry_low < entry_high(または同値)、stop < entry_low、target_today > entry_high を満たすこと。
